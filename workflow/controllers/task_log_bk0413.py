...
def ws_get_deploy_result(ws, task_log_id, ip):
    ...
    if status == 0:
        ...
        main(ws, task_log_id, redis_key_info, redis_key_result, 0)
...
def _get_ansible_result(ws, task_log_id, r_key_info, r_key_result):
    ...
    redis_api.Rs.hincrby(r_key_info, 'use_time', 1)
    time.sleep(5)
...
def main(ws, task_log_id, r_key_info, redis_key_result, status):
    try:
        ...
        # 状态-待部署
        if status == 0:
            _set_deploy(r_key_info, options)

        # 状态-正在部署
        if status == 1:
            _get_deploy_result(ws, r_key_info, redis_key_result, options)
    except Exception, e:
        print e
...
# @ext_helper.thread_method
def _get_deploy_result(ws, r_key_info, r_key_result, options):
    id = ext_helper.to_int(options['id'])
    host_ip = options['host_ip']
    playbook_name = options['playbook_name']
