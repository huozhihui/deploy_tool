...
from client.forms import CreateGroupForm


def index(request):
    title_name = _title_name()
    class_name = _class_name()
    init_date = {'state': 'present', 'system': 'no'}
    form = CreateGroupForm(initial=init_date)
...
def get_result(request):
    if request.method == 'POST':  # 当提交表单时
        form = CreateGroupForm(request.POST)  # form 包含提交的数据
...
def _ansible_api(tid, form_date):
    ...
    ansible_api.api(tid, 'group', hosts, params, ext_vars)
