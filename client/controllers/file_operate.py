...
from client.forms import CreateFileForm
...
def index(request):
    ...
    init_date = {}
    form = CreateFileForm(initial=init_date)
...
def get_result(request):
    if request.method == 'POST':  # 当提交表单时
        form = CreateFileForm(request.POST)  # form 包含提交的数据
...
def _ansible_api(tid, form_date):
    ...
    ansible_api.api(tid, 'file', hosts, params, ext_vars)
